    ...
    <div className="topbar">
      <div>
        ...
        <div className="topbar-loc-row">
          <span className="topbar-loc-name">you</span>
...
// ─── Floating Cart Bar (compact stacked pill) ────
...
export function FloatingCartOverlay() {
  ...
  // Most-recently added cart is last entry — show it on top
  const [, topCart] = entries[entries.length - 1]
  const emoji = CUISINE_EMOJI[topCart.restaurant.cuisine] ?? '🍽️'
  const topItems = Object.values(topCart.items).reduce((a, b) => a + b, 0)
    ...
    <div className={'float-bar-wrap' + (isMulti ? ' float-bar-stacked' : '')}>
      ...
      <div className="float-bar-card" onClick={openCartSheet}>
        ...
        <div className="float-bar-text">
          ...
          <div className="float-bar-sub">
            {isMulti
              ? `${totalItems} items · ${entries.length} restaurants`
              : `${topItems} item${topItems !== 1 ? 's' : ''} added`}
          </div>
        </div>
        <div className="float-bar-btn">
          {isMulti ? `View all ${entries.length}` : 'View Cart'} →
        </div>
      </div>
    </div>
  )
